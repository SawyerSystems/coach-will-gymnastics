import { type Customer, type InsertCustomer, type Parent, type InsertParent, type Athlete, type InsertAthlete, type Booking, type InsertBooking, type BlogPost, type InsertBlogPost, type Tip, type InsertTip, type Availability, type InsertAvailability, type AvailabilityException, type InsertAvailabilityException, type Admin, type InsertAdmin, type ParentAuthCode, type InsertParentAuthCode, type Waiver, type InsertWaiver, type ArchivedWaiver, type InsertArchivedWaiver, type Apparatus, type InsertApparatus, type FocusArea, type InsertFocusArea, type SideQuest, type InsertSideQuest, type BookingWithRelations, BookingStatusEnum, PaymentStatusEnum, AttendanceStatusEnum } from "@shared/schema";
import { supabase } from "./supabase-client";
import { PersistentMemStorage } from './persistent-storage';
...
export class SupabaseStorage implements IStorage {
  ...
  async createBooking(insertBooking: InsertBooking): Promise<Booking> {
    ...
    const dbBooking: any = {
      ...
      athlete1_name: insertBooking.athlete1Name,
      athlete1_date_of_birth: insertBooking.athlete1DateOfBirth,
      athlete1_allergies: insertBooking.athlete1Allergies || '',
      athlete1_experience: insertBooking.athlete1Experience,
      ...
      focus_areas: insertBooking.focusAreas,
    ...
    // Optional fields
    if (insertBooking.athlete2Name) dbBooking.athlete2_name = insertBooking.athlete2Name;
    if (insertBooking.athlete2DateOfBirth) dbBooking.athlete2_date_of_birth = insertBooking.athlete2DateOfBirth;
    if (insertBooking.athlete2Allergies) dbBooking.athlete2_allergies = insertBooking.athlete2Allergies;
    if (insertBooking.athlete2Experience) dbBooking.athlete2_experience = insertBooking.athlete2Experience;
  ...
  async getBookingWithRelations(id: number): Promise<BookingWithRelations | undefined> {
    ...
    return {
      ...booking,
      apparatus: apparatusData?.map(item => ({ id: item.apparatus.id, name: item.apparatus.name })) || [],
      focusAreas: focusAreasData?.map(item => ({ id: item.focus_areas.id, name: item.focus_areas.name })) || [],
      sideQuests: sideQuestsData?.map(item => ({ id: item.side_quests.id, name: item.side_quests.name })) || []
    };
  }
  ...
  async getAllBookingsWithRelations(): Promise<BookingWithRelations[]> {
    ...
    const apparatusByBooking = (apparatusData.data || []).reduce((acc, item) => {
      ...
      acc[item.booking_id].push({ id: item.apparatus.id, name: item.apparatus.name });
      return acc;
    }, {} as Record<number, Array<{ id: number; name: string }>>);

    const focusAreasByBooking = (focusAreasData.data || []).reduce((acc, item) => {
      if (!acc[item.booking_id]) acc[item.booking_id] = [];
      acc[item.booking_id].push({ id: item.focus_areas.id, name: item.focus_areas.name });
      return acc;
    }, {} as Record<number, Array<{ id: number; name: string }>>);

    const sideQuestsByBooking = (sideQuestsData.data || []).reduce((acc, item) => {
      if (!acc[item.booking_id]) acc[item.booking_id] = [];
      acc[item.booking_id].push({ id: item.side_quests.id, name: item.side_quests.name });
      return acc;
    }, {} as Record<number, Array<{ id: number; name: string }>>);
