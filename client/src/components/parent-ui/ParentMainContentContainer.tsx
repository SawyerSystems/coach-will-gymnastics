...
type ParentMainContentContainerProps = Omit<React.HTMLAttributes<HTMLDivElement>, 'title'> & {
...
/**
 * Main content container for parent sections following admin pattern.
 * - Dark mode: brand dark blue background (#0F0276) with subtle border
 * - Light mode: soft white/70 with blur, to match admin aesthetic
 * Place ParentCards as children inside to create the layered look.
 */
export function ParentMainContentContainer({ heading, icon, actions, className, children, ...props }: ParentMainContentContainerProps) {
  return (
    <section
      className={cn(
